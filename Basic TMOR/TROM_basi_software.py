...
'''with open('Car.png', 'r+b') as f:
	try:
		with Image.open(f) as image:
			cover = resizeimage.resize_cover(image, [32, 32])
			#cover = cv2.cvtColor(image, cv2.COLOR_BGRA2RGB)
			cover.save('Car-3PC.png', image.format)
	except:
...
def evaluate_model(pred_op, image_path):
	...
	print('Compute time was {:.10f}, and image was {:s} with name:{:s}'.format(end_time-st_time,name,image_path))
...
'''evaluate_model(z,'_Car_small.jpg')
evaluate_model(z,'P_sm.png')
evaluate_model(z,'P_2.png')
evaluate_model(z,'T_1_s.jpg')
evaluate_model(z,'H_1_s.jpg')
evaluate_model(z,'D_3_Boy_s.png')
evaluate_model(z,'D_d_small.jpg')
evaluate_model(z,'D_4_small.jpg')
evaluate_model(z,'D_5_small.png')
evaluate_model(z,'D_move_small.png')'''
